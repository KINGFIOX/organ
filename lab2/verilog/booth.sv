// Generated by CIRCT firtool-1.62.0
module Booth (
    input         clk,
    rst,
    start,
    input  [31:0] x,
    y,
    output [63:0] z,
    output        busy
);

  reg        state;
  reg        busy;
  reg [31:0] _x;
  reg [31:0] _y;
  reg [63:0] _z;
  reg [ 4:0] cnt_value;
  always @(posedge clk or posedge rst) begin
    if (rst) begin
      state <= 1'h0;
      busy <= 1'h0;
      _x <= 32'h0;
      _y <= 32'h0;
      _z <= 64'h0;
      cnt_value <= 5'h0;
    end else begin
      if (state) begin
        automatic logic [4:0] _q_T_1;
        _q_T_1 = cnt_value + 5'h1;
        state <= ~(&cnt_value);
        busy  <= ~(&cnt_value);
        if (&cnt_value) begin
          if (_x == 32'h80000000) _z <= 64'h0 - _z;
        end else begin
          automatic logic [31:0] _q_T_5 = _y >> cnt_value;
          automatic logic [31:0] _q_T_3;
          automatic logic [ 1:0] _q_1;
          _q_T_3 = _y >> _q_T_1;
          _q_1   = {_q_T_3[0], _q_T_5[0]};
          if (_q_1 == 2'h0 | (&_q_1)) _z <= {_z[63], _z[63:1]};
          else begin
            automatic logic [63:0] _GEN;
            _GEN = {_x, 32'h0};
            if (_q_1 == 2'h1) begin
              automatic logic [63:0] __z_T_3;
              __z_T_3 = _z + _GEN;
              _z <= {__z_T_3[63], __z_T_3[63:1]};
            end else if (_q_1 == 2'h2) begin
              automatic logic [63:0] __z_T_5;
              __z_T_5 = _z - _GEN;
              _z <= {__z_T_5[63], __z_T_5[63:1]};
            end
          end
        end
        cnt_value <= _q_T_1;
      end else begin
        state <= start | state;
        busy  <= start | busy;
        if (start) begin
          automatic logic [31:0] __z_T_1 = 32'h0 - x;
          _z <= {y[0] ? {__z_T_1[31], __z_T_1} : 33'h0, y[31:1]};
        end
      end
      if (~state & start) begin
        _x <= x;
        _y <= y;
      end
    end
  end  // always @(posedge)
  assign z = _z;
  assign busy = busy;
endmodule

