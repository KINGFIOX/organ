...
module Divider (
  ...
  always @(posedge clock) begin
    ...
    if (reset) begin
      ...
      if (state) begin
        ...
        if (~state | wrap) begin
          ...
          quotient <= {quotient[29:0], ~(remain[61] ? _wire_cal_T_2[61] : _wire_cal_T_4[61])};
        if (wrap) begin
          if (remain[61]) remain <= remain + extend_y;
          cnt_value <= 5'h0;
        end else begin
          if (remain[61]) remain <= _wire_cal_T_2;
          else remain <= _wire_cal_T_4;
          cnt_value <= cnt_value + 5'h1;
        end
        state <= ~wrap;
      end else begin
        if (io_start) begin
          automatic logic [61:0] _wire_cal_T = {31'h0, io_x[30:0]} - {1'h0, io_y[30:0], 30'h0};
    ...
    if (_GEN) quotient_sign <= io_x[31] ^ io_y[31];
  end  // always @(posedge)
