// Generated by CIRCT firtool-1.62.0
// Standard header to adapt well known macros for prints and assertions.

// Users can define 'PRINTF_COND' to add an extra gate to prints.
`ifndef PRINTF_COND_
  `ifdef PRINTF_COND
    `define PRINTF_COND_ (`PRINTF_COND)
  `else  // PRINTF_COND
    `define PRINTF_COND_ 1
  `endif // PRINTF_COND
`endif // not def PRINTF_COND_

// external module blk_mem_gen_1

module DCache(
  input          clock,
                 reset,
  input  [3:0]   io_data_ren,
  input  [31:0]  io_data_addr,
  output         io_data_valid,
  output [31:0]  io_data_rdata,
  input  [3:0]   io_data_wen,
  input  [31:0]  io_data_wdata,
  output         io_data_wresp,
  input          io_dev_wrdy,
  output [3:0]   io_dev_wen,
  output [31:0]  io_dev_waddr,
                 io_dev_wdata,
  input          io_dev_rrdy,
  output [3:0]   io_dev_ren,
  output [31:0]  io_dev_raddr,
  input          io_dev_rvalid,
  input  [127:0] io_dev_rdata,
  output         io_hit_r,
                 io_hit_w,
                 io_uncached
);

  wire [127:0]     _U_dsram_douta;
  wire [127:0]     _tagSram_douta;
  wire             io_uncached_0 =
    (&(io_data_addr[31:16])) & ((|io_data_ren) | (|io_data_wen));
  wire [5:0]       _GEN = {1'h0, io_data_addr[8:4]};
  reg  [2:0]       r_state;
  reg  [3:0]       ren_r;
  wire             _GEN_0 = r_state == 3'h1;
  wire             _GEN_1 = r_state == 3'h2;
  wire             _GEN_2 = r_state == 3'h3;
  wire             _GEN_3 = _tagSram_douta[23:0] == {1'h1, io_data_addr[31:9]};
  wire [3:0][31:0] _GEN_4 =
    {{_U_dsram_douta[127:96]},
     {_U_dsram_douta[95:64]},
     {_U_dsram_douta[63:32]},
     {_U_dsram_douta[31:0]}};
  `ifndef SYNTHESIS
    always @(posedge clock) begin
      if ((`PRINTF_COND_) & (|r_state) & ~_GEN_0 & ~_GEN_1 & _GEN_2 & _GEN_3 & ~reset)
        $fwrite(32'h80000002, "dataOutVec=%x\n", _U_dsram_douta);
    end // always @(posedge)
  `endif // not def SYNTHESIS
  wire             _GEN_5 = r_state == 3'h4;
  wire [3:0][31:0] _GEN_6 =
    {{io_dev_rdata[127:96]},
     {io_dev_rdata[95:64]},
     {io_dev_rdata[63:32]},
     {io_dev_rdata[31:0]}};
  wire             _GEN_7 = _GEN_5 & io_dev_rvalid;
  wire             _GEN_8 = ~(|r_state) | _GEN_0;
  wire             _GEN_9 = _GEN_5 & io_dev_rvalid;
  wire             _GEN_10 = ~(|r_state) | _GEN_0 | _GEN_1 | _GEN_2;
  reg  [1:0]       w_state;
  reg  [3:0]       wen_r;
  reg  [31:0]      wdata;
  reg              wr_resp_REG;
  wire             wr_resp = ~wr_resp_REG & io_dev_wrdy;
  wire             _GEN_11 = w_state == 2'h0;
  wire             _GEN_12 = w_state == 2'h1;
  wire             _GEN_13 = _GEN_12 & io_dev_wrdy;
  wire             _GEN_14 = _GEN_11 | ~_GEN_13;
  wire             _GEN_15 = ~io_uncached_0 & ~(|(io_data_addr[1:0])) & _GEN_3;
  wire             _GEN_16 =
    ~_GEN_11 & _GEN_12 & io_dev_wrdy & _GEN_15 | ~_GEN_10 & _GEN_9;
  wire             _GEN_17 = _GEN_14 ? _GEN_10 | ~_GEN_7 : _GEN_15 | _GEN_10 | ~_GEN_7;
  wire             _GEN_18 = w_state == 2'h2;
  always @(posedge clock) begin
    if (reset) begin
      r_state <= 3'h0;
      ren_r <= 4'h0;
      w_state <= 2'h0;
      wen_r <= 4'h0;
      wdata <= 32'h0;
    end
    else begin
      if (|r_state) begin
        if (_GEN_0) begin
          if (io_dev_rrdy)
            r_state <= 3'h2;
        end
        else if (_GEN_1) begin
          if (io_dev_rvalid)
            r_state <= 3'h0;
        end
        else if (_GEN_2) begin
          if (_GEN_3)
            r_state <= 3'h0;
          else if (io_dev_rrdy)
            r_state <= 3'h4;
        end
        else if (_GEN_7)
          r_state <= 3'h0;
      end
      else if (|io_data_ren)
        r_state <= {1'h0, ~(io_uncached_0 | (|(io_data_addr[1:0]))), 1'h1};
      if (~(|r_state) & (|io_data_ren))
        ren_r <= io_data_ren;
      if (_GEN_11) begin
        if (|io_data_wen)
          w_state <= 2'h1;
      end
      else if (_GEN_12) begin
        if (io_dev_wrdy)
          w_state <= 2'h2;
      end
      else if (_GEN_18 & wr_resp)
        w_state <= 2'h0;
      if (_GEN_11 & (|io_data_wen)) begin
        wen_r <= io_data_wen;
        wdata <= io_data_wdata;
      end
    end
    wr_resp_REG <= io_dev_wrdy;
  end // always @(posedge)
  blk_mem_gen_1 tagSram (
    .clka  (clock),
    .wea   (_GEN_16),
    .addra (_GEN),
    .dina  (_GEN_17 ? 128'h0 : {105'h1, io_data_addr[31:9]}),
    .douta (_tagSram_douta)
  );
  blk_mem_gen_1 U_dsram (
    .clka  (clock),
    .wea   (_GEN_16),
    .addra (_GEN),
    .dina  (_GEN_17 ? 128'h0 : io_dev_rdata),
    .douta (_U_dsram_douta)
  );
  assign io_data_valid = ~_GEN_8 & (_GEN_1 ? io_dev_rvalid : _GEN_2 ? _GEN_3 : _GEN_9);
  assign io_data_rdata =
    _GEN_8
      ? 32'h0
      : _GEN_1
          ? (io_dev_rvalid ? io_dev_rdata[31:0] : 32'h0)
          : _GEN_2
              ? (_GEN_3 ? _GEN_4[io_data_addr[3:2]] : 32'h0)
              : _GEN_7 ? _GEN_6[io_data_addr[3:2]] : 32'h0;
  assign io_data_wresp = ~(_GEN_11 | _GEN_12) & _GEN_18 & wr_resp;
  assign io_dev_wen = _GEN_14 ? 4'h0 : wen_r;
  assign io_dev_waddr = _GEN_14 ? 32'h0 : io_data_addr;
  assign io_dev_wdata = _GEN_14 ? 32'h0 : wdata;
  assign io_dev_ren =
    (|r_state)
      ? (_GEN_0
           ? (io_dev_rrdy ? ren_r : 4'h0)
           : _GEN_1 | ~_GEN_2 | _GEN_3 ? 4'h0 : {4{io_dev_rrdy}})
      : 4'h0;
  assign io_dev_raddr =
    (|r_state)
      ? (_GEN_0
           ? (io_dev_rrdy ? io_data_addr : 32'h0)
           : _GEN_1 | ~_GEN_2 | _GEN_3 | ~io_dev_rrdy
               ? 32'h0
               : {io_data_addr[31:4], 4'h0})
      : 32'h0;
  assign io_hit_r = ~(~(|r_state) | _GEN_0 | _GEN_1) & _GEN_2 & _GEN_3;
  assign io_hit_w = ~_GEN_11 & _GEN_13 & _GEN_15;
  assign io_uncached = io_uncached_0;
endmodule

